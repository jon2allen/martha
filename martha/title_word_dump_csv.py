...
def insert_dict( word, dict):
    if word in dict:
        ...
        dict[word] = 1
    return
def get_titles_from_db(num_entries):
    my_db = martha_db(load_var('DB_USERNAME'), load_var('DB_PASSWORD'))
    res = my_db.run_query_get_all(num_entries)
    return res

cat_list = ["entree", "baked item", "dessert", "side-dish", "drink", "tapa", "soup"]

def main_loop():
    title_dict = {}
    res_list = get_titles_from_db(2000)
    print("\"" + "recipe" + "\"," + "\"" + "Category" + "\"")
    for r in res_list:
        print("\"" + r[1].strip() + "\",\"" + random.choice(cat_list) + "\"")
